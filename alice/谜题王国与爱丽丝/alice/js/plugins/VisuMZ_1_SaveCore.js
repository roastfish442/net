 ...
 * @parent General
 * @type select
 * @option 标准: 可以自由地在任何槽位保存。
 * @value standard
 * @option 锁定槽位: 在新游戏时选择一个专用的槽位。
 * @value locked
 * @option 单一槽位: 游戏只能使用一个保存槽位。
 * @value single
 * @desc 选择游戏的保存样式。这些选项中的一些可能会改变其他插件参数设置。
 * @default standard
 * 
 ...
 * @on 本地文件
 * @off Forage 键
 * @desc 在客户端运行游戏时，是通过文件的本地模式保存，还是将保存存储到 Forage 键？
 * @default true
 * 
 ...
 * @parent General
 * @type select
 * @option 自动保存文件：专用于自动保存的文件。
 * @value file0
 * @option 当前文件：覆盖当前保存文件。
 * @value current
 * @option 自动保存文件 + 当前文件：同时进行上述两种保存。
 * @value both
 * @desc 选择自动保存类型。
 * 需要数据库 => 系统 1 => [x] 启用自动保存。
 * @default file0
 ...
 * @text 最新颜色
 * @parent General
 * @desc 使用 #rrggbb 表示自定义颜色，或使用窗口皮肤中的文本颜色。
 * @default #f49ac1
 *
