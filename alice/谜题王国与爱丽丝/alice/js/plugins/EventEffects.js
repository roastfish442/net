...
/*:zh
 * @target MV MZ
 * @plugindesc [Ver 1.1.3] 为事件和玩家设置各种效果
 * @author Sasuke KANNAZUKI
 *
 * @help 本插件不提供插件命令。
 ...
 *
 * ■ 改变色调
 * this.tint(Array[red,green,blue,gray], 帧数 ,waitFlag);
 * 颜色：-255～255，60帧 = 1秒。
 * 例：
 * this.tint([255,255,255,0], 20, true);  # 在20帧内变白
 * this.tint([0,0,0,0], 20, true);        # 在20帧内恢复正常
 *
 * ■ 改变不透明度
 * this.opaque(opacity, 帧数, waitFlag);
 * this.opaque(128, 20, true);        # 在20帧内变为半透明
 *
 * ■ 同时设置色调与不透明度
 * this.tint2(Array[red,green,blue,gray], opacity, 帧数 ,waitFlag);
 * this.tint2([64,64,64,0], 20, 128, false); 
 *   # 在20帧内变暗并半透明
 *
 * ■ 色调的简写
 * this.tintB(frame, waitF);   # 变为黑色([-255,-255,-255,0])
 * this.tintW(frame, waitF);   # 变为白色([255,255,255,0])
 * this.tintN(frame, waitF);   # 恢复正常([0,0,0,0])
 * 以上函数的 frame 和 waitF 参数可省略。
 * 默认值：frame=60，waitF=true。
 *
 * ■ 气泡图标
 * this.balloon(num, wait);   # num:1～10, wait:true/false
 * num: 1/感叹号, 2/问号, 3/音符, 4/爱心, 5/生气,
 *      6/汗, 7/蜘蛛网, 8/沉默, 9/灯泡, 10/Zzz
 * 例：
 * this.balloon(1, true);     # 感叹号图标（等待）
 * this.balloon(2, false);    # 问号图标（不等待）
 *
 * ■ 上下颠倒
 * this.setUpSideDown();        # 显示为上下颠倒
 * this.resetUpSideDown();      # 取消上下颠倒
 * 注：通过旋转180度和改变偏移实现。
 *
 * ■ 设置角度
 * this.setAngle(90);           # 向右旋转90度
 * this.setAngle(-90);          # 向左旋转90度
 * this.setAngle(0);            # 恢复原始角度
 *
 * ■ 设置偏移
 * this.setOffsets(10,20);      # X+10，Y+20
 * this.setOffsets(0,0);        # 恢复原位置
 *
 * ■ 进一步设置偏移（Ver 1.1新增）
 * this.moveOffsets(25,-15);     # 从当前偏移再移动 X+25, Y-15
 * this.setOffsetFwd(8);        # 朝角色朝向设置偏移 8
 * this.moveOffsetFwd(8);       # 从当前偏移朝前移动 8
 *
 * ■ 综合设置
 * this.setEffects(ox,oy,usd,angle);
 * ox,oy: 偏移量；usd: 是否上下颠倒(true/false)；angle: 角度（360为一周）
 * 例：
 * this.setEffects(-15,20,false,45); # 偏移(-15,20)，顺时针旋转45度
 * 
 * ■ 优先级类型
 * this.setPriorityType(priority);  # 优先级 0:下层 1:正常 2:上层
 * 注：此为预设函数。
 *
 * ■ 角色交叉淡入更换
 * this.setGrid2(dir, pattern, frames, wait);
 * this.setImageGrid2(name, index, dir, pattern, frames, wait);
 * dir=2,4,6,8；pattern=0,1,2；frames(60帧=1秒)，wait: 是否等待完成
 * name=角色图像名称，index=角色索引(0～7)
 *
 * ■ 设置偏移与视口
 * this.setOffset2(dx, dy[, slx, sly, srx, sry]);
 *   (dx, dy): 偏移坐标
 *   (slx, sly) - (srx, sry): 角色位图的视口范围
 *    | 省略这4个参数时，默认使用整个角色位图范围。
 * this.resetOffset2();
 *   重置 setOffset2 设置。
 *
 * 【许可协议】
