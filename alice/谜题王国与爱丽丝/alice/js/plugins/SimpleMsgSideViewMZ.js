...
// 2022.May.15 Ver1.2.0 Resolve confliction with Keke_SpeedStarBattle.js
...
/*:ja
 * @target MZ
 * @plugindesc [Ver1.2.0]サイドビューバトルでスキル/アイテムの名前のみ表示
 * @author 神無月サスケ
 *
 * @param displayAttack
 * @text 通常攻撃も表示
 ...
 * - このプラグインを Keke_SpeedStarBattle.js と併用して使う場合、
 *   このプラグインを後に置いて下さい。
 *
 * ■ライセンス表記
...
/*:zh
 * @target MZ
 * @plugindesc [Ver1.2.0] 在侧视战斗中仅显示技能或道具名称
 * @author Sasuke KANNAZUKI
 *
 * @param displayAttack
 * @text 显示普通攻击
 ...
 * 本插件适用于 RPG Maker MZ。
 *
 * 启用此插件后，战斗时将不再显示详细战斗日志，
 * 仅显示使用的技能或道具名称。
 *
 * 【扩展功能：显示一行说明】
 * 若启用“显示一行说明”选项，
 * 将在技能或道具名称下显示对应的简短描述。
 * 说明文本可在技能或道具的备注栏中输入以下格式：
 * <simpleDesc:描述内容>
 * 若未填写此标签，将自动使用技能/道具的“消息第二行”内容作为说明。
 *
 * 【注意事项】
 * - 支持正面视角战斗，但在该模式下不会显示友方受到的伤害。
 * - 因移除了日志显示，仅显示技能名称，战斗速度会略微加快。
 * - 若与插件「Keke_SpeedStarBattle.js」同时使用，
 *   请将本插件置于其下方。
 *
 * 【许可证】
 * 本插件基于 MIT 许可证发布。
 * 您可以自由修改、再分发或用于任何类型的游戏（包括商业与成人游戏）。
 * http://opensource.org/licenses/mit-license.php
 */
