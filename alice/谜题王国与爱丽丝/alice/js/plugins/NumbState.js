...
/*:zh
 * @target MV MZ
 * @plugindesc 使角色进入麻痹状态的插件（Ver1.2）
 * @author Sasuke KANNAZUKI
 *
 * @param dispMsgType
 * @text 显示消息类型
 ...
 * @parent dispMsgType
 * @text 麻痹消息
 * @desc 当角色因麻痹而无法行动时显示的文字。
 * %1 会被替换为角色名，%2 会被替换为状态名。
 * @type string
 * @default %1因麻痹而无法行动！
 *
 * @help 本插件不提供插件指令。
 * 本插件可在 RPG Maker MV 与 MZ 下运行。
 *
 * 本插件可创建一种名为“麻痹（numb）”的状态，与麻痹（paralysis）不同。
 *
 * 【概要】
 * “麻痹”状态下，虽然可以输入指令，但有一定概率无法行动。
 *
 * 【状态备注说明】
 * 要启用此状态，请在状态的备注栏中写入以下内容：
 *   <numb_rate:35>  # 代表有 35% 的几率无法行动
 *
 * 【关于“显示消息类型”选项】
 * 当角色因麻痹而无法行动时，可以选择显示的消息来源：
 * - 若选择“使用本插件的选项”，则会显示此插件中设置的“麻痹消息”。
 * - 若选择“使用数据库中的状态消息”，则会显示状态中
 *   “若状态持续时显示的消息”。
 *   如果选择此项，当角色未受麻痹影响时，不会显示任何消息。
 *
 * 当你制作多个可能触发麻痹的状态时，
 * 选择后一种方式会更方便。
 *
 * 【注意】
 * 建议麻痹状态的“行动限制”设置为“无（可输入指令）”。
 *
 * 【许可证】
 * 本插件基于 MIT 许可证发布。
 * http://opensource.org/licenses/mit-license.php
