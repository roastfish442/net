//=============================================================================
// Foreground.js (Ver1.1)
//=============================================================================
// [History]
// 2015.Nov. 7  1.0.0 First Release
// 2018.Sep.19  1.0.1 Fix bug when 'event command -> test' becomes error
// 2019.Dec.08  1.1.0 Update to run under also MZ

/*:
 * @target MV MZ
 * @plugindesc display foreground tiling sprite(Ver1.1)
...
/*:ja
 * @target MV MZ
 * @plugindesc マップに合わせてスクロールする近景の設定(Ver1.1)
 * @author 神無月サスケ
 * 
 * @noteParam fgName
 * @noteRequire 1
 ...
 *
 * ■ライセンス表記
 * プラグイン作成にあたりご指導くださったYoji Ojima様に感謝します。
 ...
 * 
 * @help 本插件不提供插件命令。
 * 本插件可在 RPG Maker MV 与 MZ 中运行。
 *
 * 【概要】
 * “前景（foreground）”类似于远景（parallax），
 * 但会显示在地图的最上层。
 * 前景图像文件应放置在 **img/parallaxes/** 文件夹内。
 *
 * 【地图备注设置】
 * 在地图的备注（Note）中填写以下 5 个参数：
 *
 * <fgName:文件名>
 *  指定要作为前景使用的文件名（无需扩展名）。
 *  若文件名以 “!” 开头，则前景的滚动为“零视差”（不随地图移动）。
 *  图像文件需放在 img/parallaxes/ 文件夹中。
 *
 * <fgLoopX:数字>
 *  是否在 X 方向循环滚动（0: 否 / 1: 是）。
 *  若未填写，默认为 0。
 *
 * <fgLoopY:数字>
 *  是否在 Y 方向循环滚动（0: 否 / 1: 是）。
 *  若未填写，默认为 0。
 *
 * <fgSx:数字>
 *  X 方向的滚动速度。
 ...
 * 【示例】
 * <fgName:sample1><fgLoopX:1><fgLoopY:1>
 * → 显示 sample1.png，并在 X 与 Y 方向上循环滚动。
 *
 * <fgName:!sample1>
 * → 显示 !sample1.png，前景固定，不随地图移动。
 *
 * 【说明】
 * - 可与天气特效（Weather）共存，前景位于天气图层下方。
 * - 支持 MV 与 MZ。
 *
 * 【许可】
 * 感谢 Yoji Ojima 的技术指导。
 * 本插件以 MIT 许可证发布，可自由使用、修改与再分发。
 * http://opensource.org/licenses/mit-license.php
 */
