 ...
 * @target MZ
 * @url https://github.com/effekseer/EffekseerForRPGMakerMZ_Ex
 * @plugindesc Effekseer 扩展插件 v1.62d - 1.03
 * @author Effekseer
 *
 * @help
 * 这是一个用于扩展 Effekseer 功能的插件。
 * 可提升性能表现。
 *
 * 当使用的特效数量过多而无法全部显示时，可通过调整参数解决。
 *
 * @param InstanceMaxCount
 * @text 实例最大数量
 * @desc 一次可同时显示的特效实例的最大数量。
 * @type number
 * @default 10000
 *
 * @param SquareMaxCount
 * @text 精灵最大数量
 * @desc 一次可同时显示的特效精灵的最大数量。
 * @type number
 * @default 10000
 *
 * @param DistortionEnabled
 * @text 启用失真效果
 * @desc 是否启用特效的失真效果（Distortion）。
 * @type boolean
 * @default false
 *
 * @help
 * 【概要】
 * 本插件用于在 RPG Maker MZ 中扩展 Effekseer 的渲染功能，
 * 提高显示上限并可启用特殊的失真渲染模式。
 *
 * 【使用方法】
 * 1. 请确保项目中已正确启用 Effekseer。
 * 2. 将本插件置于 EffekseerForRPGMakerMZ.js 之后。
 * 3. 可在插件参数中调整实例与精灵的最大数量。
 *
 * 【参数说明】
 * ● 实例最大数量：
 *    - 控制同屏可显示的特效实例数。
