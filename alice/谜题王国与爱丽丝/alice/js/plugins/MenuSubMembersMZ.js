...
/*:zh
 * @target MZ
 * @plugindesc 在菜单与地图上显示“同行者”（非战斗成员）
 * @author Sasuke KANNAZUKI
 *
 * @param subMemberIdVal1
 * @text 同行者1用变量ID
 * @desc 指定存储同行者1的角色ID的变量ID
 * @type variable
 * @default 1
 *
 * @param subMemberIdVal2
 * @text 同行者2用变量ID
 * @desc 指定存储同行者2的角色ID的变量ID
 * @type variable
 * @default 2
 *
 * @param subMemberIdVal3
 * @text 同行者3用变量ID
 * @desc 指定存储同行者3的角色ID的变量ID
 * @type variable
 * @default 3
 *
 * @param subMemberIdVal4
 * @text 同行者4用变量ID
 * @desc 指定存储同行者4的角色ID的变量ID
 * @type variable
 * @default 4
 *
 * @param subMemberText
 ...
 *
 * 本插件可设置“同行者”。
 * 所谓同行者，是不会参加战斗、也不会作为角色出现在菜单中，
 * 但会以“同行者”的身份显示在菜单和队列中的角色。
 *
 * ■ 概要
 * 同行者最多可设置4人。
 * 如果不想设置同行者，请将对应变量的值设为0。
 *
 * 如果不是使用同行者，而是想增加普通角色的显示人数，
 * 请通过“队列追加人数”选项进行设置。
 * 默认值为4，此时最多可显示8名角色。
 * 该设置与同行者人数无关。
 *
 * 当角色总数超过可显示人数时，末尾的角色将不会显示；
 * 但同行者则始终全部显示。
 *
 * ■ 注意
 * 本插件仅适用于默认菜单画面。
 * 如果使用其他插件（如 AltMenuScreen.js）修改了菜单布局，
 * 本插件将无法正常使用。
 *
 * ■ 许可协议
 * 本插件基于 MIT 许可协议发布，
