...
/*:zh
 * @target MZ
 * @plugindesc [Ver1.0.1] 从菜单中打开的物品强化（合成）系统
 * @author Sasuke KANNAZUKI
 *
 * @param isDebugMode
 * @text 调试模式？
 * @desc 若选择“是”，游戏启动时将在控制台输出合成配方列表。发布时请选择“否”。
 * @type boolean
 * @on 是
 * @off 否
 * @default false
 ...
 * @parent playType
 * @text 成功时SE
 * @desc 合成成功时播放的 SE 文件名（仅当类型为 SE 时有效）。
 * @default Chime2
 * @require 1
 * @dir audio/se/
 * @type file
 ...
 * @parent playType
 * @text 成功时ME
 * @desc 合成成功时播放的 ME 文件名（仅当类型为 ME 时有效）。
 * @default Item
 * @require 1
 * @dir audio/me/
 * @type file
 ...
 * @parent dummy4text
 * @text 确认强化装备
 * @desc “%1正在装备该物品，确定要直接合成吗？”文字。%1会被替换为角色名。
 * @type string
 * @default %1正在装备该物品。是否在装备状态下直接合成？
 *
 * @param textSureToCombineNormal
 * @parent dummy4text
 * @text 确认强化普通物品
 ...
 * 请在该物品或装备的备注栏中写入如下格式：
 * <Material:xxxx>
 * xxxx 是字符串。根据物品内容命名，例如 SolarStone、MoonStone 等，
 * 请确保名称唯一。
 * 这个字符串将作为强化候选物品备注的关键字。
 *
 * 例：
 * <Material:SolarStone>
 * <Material:MoonStone>
 ...
 * <xxxx:20>
 * <xxxx:1,15>
 * 这里的 xxxx 请替换为 SolarStone、MoonStone 等
 * 作为合成素材定义的字符串。
 * 当填写数值（如 20）时，
 * 会强化为相同类别（物品或装备）ID 对应的物品。
 * 若要指定不同类别，可以写成 <xxxx:1,15>，
 * 这表示武器 ID 为 15。（0=物品，1=武器，2=防具）
 *
 * 例：
 * <SolarStone:20>
 * <MoonStone:1,15>
