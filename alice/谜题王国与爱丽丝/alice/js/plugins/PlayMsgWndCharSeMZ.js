...
/*:zh
 * @target MZ
 * @plugindesc [Ver1.1.0] 在消息窗口中为每个字符播放音效（SE）
 * @author Sasuke KANNAZUKI
 *
 * @param default SE
 * @text 地图默认SE编号
 * @desc 每次进入地图时初始化为此值。
 * 请输入0～5之间的整数。设置为0则不播放音效。
 * @type number
 * @max 5
 * @min 0
 * @default 1
 ...
 * @param advanced
 * @text 高级设置
 * @desc 除非与其他插件冲突，否则无需更改此项。
 *
 * @param prefix
 * @parent advanced
 * @text SE替代前缀
 * @desc 若 \SE[] 的格式与其他插件冲突，可改为其他字符串。
 * @type string
 * @default SE
 *
 * @help 本插件没有插件命令。
 ...
 * 实现“嘟嘟嘟……”的对话效果。
 *
 * 可设置最多5种不同的音效，根据情况自由切换。
 *
 * 【用法说明】
 * 在消息窗口的文本中可使用以下格式切换SE：
 * \SE[0]：停止播放音效
 * \SE[1]：播放编号1的音效
 * \SE[2]：播放编号2的音效
 * 同理，也可使用 \SE[3]、\SE[4]、\SE[5]
 *
 * 该设置会在“地图切换”或“场景切换”时重置为默认值。
 * 注意：打开或关闭菜单也会触发“场景切换”。
 *
 * 若在文本中使用 \>，
 * 无论当前间隔（interval）设定为何，
 * 都会强制播放一次SE。
 *
 * 【许可条款】
 * 本插件基于 MIT 许可证发布。
 * 您可以自由修改、再发布或用于商业、成人等任何用途。
 * http://opensource.org/licenses/mit-license.php
 */
