 ...
 * @text 遇敌率公式
 * @desc 遇敌率计算公式（默认值为1）
 * 1：步数 + 步数 + 最小值　2：步数 + 最小值　其他请参见帮助
 * @default 1
 *
 * @help 本插件没有插件命令。
 *
 * 你可以设置计算随机遇敌的最小步数和遇敌率的公式。
 *
 * 如果指定变量的值为 0，则进入地图后可能立即发生遭遇战。
 * 你可以随时更改最小步数，但在下一次遇敌之前，会继续沿用之前的步数。
 * 因此，如果希望立即生效，请在修改后切换地图。
 *
 * 实际的步数是随机选择的，范围是 0 到地图中设置的“敌人出现步数”之间。
 *
 * 如果在公式（formula）中设置为 3，
 * 则会直接使用变量中指定的最小步数，省略随机要素。
 *
 * 如果在公式（formula）中设置为 4～6，
 * 则会使用该数值作为最小步数，而不再读取变量。
 * 当你不想为每张地图都设置变量，或不想消耗变量时很实用。
 * 4：步数 + 步数 + 最小值
 * 5：步数 + 最小值
 * 6：固定值步数
 *
 * 特别鸣谢 kuro(http://fanblogs.jp/tabirpglab/)
 *
 * 本插件适用于 RPG Maker MV 与 MZ。
 * 使用无需申报，无需署名，可修改，可商用。
 * 作者不对任何可能发生的问题承担责任，请理解后使用。
 */
